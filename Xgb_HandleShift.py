...
SAVE_PATH = "Xgb_DistributionShift_result"
...
def predict_on_test_tail(feature_selections):
    """
    Load the trained model and use it to predict the last 1818 rows of X_test_2.
    Saves predictions as 'preds_2.npy'.
    """
    model_path = os.path.join(SAVE_PATH, "final_model_2.json")
    if not os.path.exists(model_path):
        raise FileNotFoundError("Trained model not found: final_model_2.json")
    ...
    X_test_tail = X_test_2.iloc[202:].copy()

    # Replace with your actual selected top fisher features
    top_fisher = feature_selections

    # Apply the same winsorization and scaling used during training
    X_test_tail_iqr = winsorize_iqr(X_test_tail[top_fisher])
    X_train = pd.read_csv("data/X_train.csv")
    X_train_iqr = winsorize_iqr(X_train[top_fisher])
    scaler = RobustScaler().fit(X_train_iqr)
    X_test_tail_scaled = scaler.transform(X_test_tail_iqr)

    # Make predictions and save
    preds_tail = model.predict_proba(X_test_tail_scaled)
    np.save(os.path.join(SAVE_PATH, "preds_2.npy"), preds_tail)
    print("Saved predictions to preds_2.npy")
...
def main():
    ...
    X = pd.concat([X, X_test_2_subset], axis=0).reset_index(drop=True)
    y = pd.concat([y, y_test_2_reduced], axis=0).reset_index(drop=True).squeeze()
    ...
    # === SMOTE for tuning input ===
    X_fisher = winsorize_iqr(X_train_sub[top_fisher])
    scaler = RobustScaler().fit(X_fisher)
    X_fisher_scaled = scaler.transform(X_fisher)
    smote = SMOTE(random_state=42, k_neighbors=3)
    X_fisher_res, y_fisher_res = smote.fit_resample(X_fisher_scaled, y_train_sub)
    ...
    best_params = tune_model(X_fisher_res, y_fisher_res, base_model, param_grids)

    # === Covariate + Label Shift Training ===
    # Train final model using Fisher top-K features
    # Apply covariate shift correction using domain classifier
    # Estimate label shift via confusion matrix reweighting
    # Train with adjusted labels and sample weights
    train_with_shift_correction(X_train_sub, y_train_sub, X_val, y_val, top_fisher, best_params)
    predict_on_test_tail(top_fisher)
